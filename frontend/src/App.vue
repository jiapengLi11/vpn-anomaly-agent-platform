    ...
    <main class="workspace-main">
      <header class="workspace-topbar">
        ...
        <div class="topbar-actions">
          <span class="connection-status" :class="{ demo: store.demoMode }"><i />{{ route.path === '/knowledge' ? '资料与问答' : route.path === '/agent' ? '编排策略台' : route.path === '/billing' ? '沙箱计费账户' : store.demoMode ? '离线演示 · 只读' : store.backendHealth?.status === 'UP' ? '后端已连接' : '等待服务连接' }}</span>
      ...
      <div v-if="store.demoMode && !['/knowledge','/agent','/billing'].includes(route.path)" class="mode-banner"><AppIcon name="file" /><span>正在展示合成流量的离线分析产物。此模式不会上传文件或调用外部模型。</span><button @click="switchMode(false)">连接服务<AppIcon name="arrow" /></button></div>
...
const navigation = [
  ...
  { path: "/agent", label: "Agent 编排", icon: "workflow" },
