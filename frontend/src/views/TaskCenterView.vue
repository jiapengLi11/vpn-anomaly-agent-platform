<template>
  <section class="page-enter">
    <div class="page-heading"><div><h1>{{ reports ? '研判报告' : '任务中心' }}</h1><p>{{ reports ? '按任务查看分析结论、候选通信与可追溯证据。' : '上传样本、追踪分析进度，所有任务集中管理。' }}</p></div><el-button type="primary" size="large" @click="store.uploadOpen = true"><AppIcon name="plus" />新建分析</el-button></div>
    <section class="panel">
      <div v-if="!reports" class="filter-tabs" role="group" aria-label="按状态筛选"><button v-for="item in tabs" :key="item.value" :class="{ selected: status === item.value }" @click="status = item.value">{{ item.label }}<span>{{ item.count ?? '--' }}</span></button></div>
      ...
      <div v-loading="loading"><TaskTable :items="items" :empty-text="reports ? '暂无可查看的研判报告' : '没有匹配的分析任务'" /></div>
      <div class="pagination-bar"><span>共 {{ total }} 条{{ reports ? '报告' : '任务' }}</span><el-pagination v-model:current-page="page" :page-size="10" :total="total" layout="prev, pager, next" @current-change="load" /></div>
    </section>
    <div class="list-tip"><AppIcon name="file" /><div><strong>{{ reports ? '每一份报告都有依据' : '不知道从哪里开始？' }}</strong><p>{{ reports ? '候选流不等于恶意流量。进入报告后可查看当前推理后端与筛选依据。' : '点击右上角“新建分析”上传 PCAP，完成后点击“查看报告”继续。' }}</p></div></div>
  </section>
</template>
<script setup>
...
const reports = computed(() => route.path === "/reports");
const status = ref(reports.value ? "SUCCESS" : ""), model = ref(""), keyword = ref(""), page = ref(1);
