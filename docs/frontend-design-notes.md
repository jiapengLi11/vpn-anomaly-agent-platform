...
移动端把横向六节点轨迹改成纵向时间线，指标卡变为两列，审计卡片单列排列。浏览器自动化分别在 1440px
...
新迭代保留原有视觉体系，在回答区呈现标注清楚的生成中草稿，发送按钮在请求期间替换为“停止生成”。
检索来源先于答案出现，让等待过程有明确状态。完成时用通过引用 ID 校验的答案替换草稿；失败则撤回，
避免把半截答案当成可靠证据。会话控制集中放在问答上方，不新增一级导航。

![真实流式生成过程](assets/knowledge-qa-streaming.png)

本机记忆为主动开启的 7 天记录，支持刷新恢复、切换和清除；不把它称为长期记忆或知识库。
实践中 CDP 无法读取已取消读取器的 SSE 响应体，浏览器测试改为检查草稿可见状态和最终保存结果，
协议分帧则由独立 Node/Python 测试覆盖，避免为了获取测试结果重复触发付费请求。

右侧来源区增加小型“工程评测基线”，从生成的 JSON 展示 Hit@1、MRR 和场景数，并紧邻标注
“自编开发集，不代表真实业务准确率”。这让面试演示可以从界面进入评测报告，同时不把回归数据包装成生产指标。
