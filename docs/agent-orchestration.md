# Agent 编排中心：从工具建议到受控执行

这个页面用于回答一个常见 Agent 面试问题：当工具逐渐增多时，不能把完整工具定义都塞进 Prompt，也不能让模型直接决定权限、依赖和并行关系。
...
访问 `http://127.0.0.1:8088/#/agent`。GitHub Pages 使用冻结的只读计划，页面会显示 `公开快照 · 只读`，不会调用本机或外部模型。
