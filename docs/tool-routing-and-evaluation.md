...
| L2 回答质量 | 事实支持度、完整性、不确定性、拒答 | 只有引用存在性检查 | 双人标注；LLM Judge 仅辅助并校准一致性 |
| L3 Agent 链路 | 路由、短路、降级、Claim Gate、trace | 合成端到端回归 | MCP 集成、人审中断与恢复 |
...
下一阶段为每个问题标注可支持事实、必需引用、允许的保留意见和不可输出结论。人工评价至少拆成：
